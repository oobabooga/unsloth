...
async function placeWindow(
  ...
  // Center against the size just requested. Tauri's own center() reads the size
  // back from the window, which on Linux is the GTK configure-event cache
  // behind #7659, so straight after setSize it centers the previous size.
...
async function showSetupWindow(isCurrent: WindowLayoutGuard): Promise<void> {
  ...
  if (!isCurrent()) return;
  // Drop any app-mode minimum, which would otherwise floor the smaller setup
  // window at 900x600.
  ...
  if (!measured) return;
  // The setup window is not resizable, so a work area shorter than its
  // preferred height would leave the wizard's bottom permanently unreachable.
...
async function enforceWindowSizeBounds(
  ...
  if (!isCurrent()) return;
  // Maximized and fullscreen windows are sized by the window manager, and
  // resizing them here would silently drop them out of that state.
...
async function applyAppWindowLayout(
  ...
  if (hasInitializedAppLayout && hasSavedState) {
    ...
    if (!isCurrent()) return;
    // Restoring can land the window on a different monitor than the hidden one
    // resolved to, so read the work area that now applies.
  ...
  // a Resized event and overwrite the plugin's cached saved size. Clamp the
  // minimum to the work area so it can never forbid shrinking a window that is
  // taller than the panel allows.
