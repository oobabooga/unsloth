...
test("fits a 1366x768 panel at 125% scaling above the taskbar", () => {
  ...
  // The window still clears the taskbar, unlike the 600 the base pins.
...
test("relaxes a minimum that does not fit, keeping room to resize", () => {
  for (const [panel, scaleFactor] of [
    ...
    // A minimum equal to the maximum would make the window unresizable.
    assert.ok(
      minimum.height < maximum.height,
...
test("fits the non-resizable setup window to the available area", () => {
  ...
  // A roomy panel keeps the preferred size untouched.
  assert.deepEqual(
    fitWindowSize(
      PREFERRED_SETUP_WINDOW_SIZE,
...
test("centers against the work area of the monitor it is on", () => {
  ...
  // A window wider than the work area pins to its origin instead of hanging
  // off the left edge.
  assert.deepEqual(
