...
def _purge_incomplete_blobs(
    ...
    """Delete matching ``*.incomplete`` blobs beneath *entry*.
    ...
    A failed deletion must be visible to the marker writer. Otherwise an HTTP
    marker could certify a sparse Xet partial that survived the purge.
    """
...
class DownloadRegistry:
    ...
    def __init__(self, max_terminal: int = 64) -> None:
        ...
        # Repository-wide owners cover cache writers that are not Model Hub
        # jobs but share its cache, currently the STT sidecars. Publishing the
        # owner under this same lock closes both claim interleavings: either the
        # Model Hub job is already visible, or its later claim sees the owner.
    ...
    def claim_repository_owner(self, repo_id: str, owner: object) -> tuple[bool, str]:
        ...
        The reservation conflicts with full snapshots, every GGUF variant, and
        deletes regardless of blob overlap. Different repositories remain
        independent. The opaque *owner* is required for release so a stale STT
        run cannot release a newer run's claim.
        """
        ...
        with self._lock:
            ...
            for key, job in self._jobs.items():
                if _repo_of_key(key) != repo or job.state not in _ACTIVE_STATES:
                    continue
                # Scan the complete job table as well as _repo_active because a
                # retry handoff temporarily releases its active slot.
