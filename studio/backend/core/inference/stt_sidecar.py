...
def _prepare_stt_cache_for_http(repo: str, hub_cache: Path) -> None:
    """Prepare *repo* for sequential HTTP and verify its marker readback.

    The caller already holds the repository ownership claim. A missing or
    unreadable marker is a hard failure because proceeding could append HTTP
    bytes to a sparse partial produced by Xet or hf_transfer.
    """
...
def _active_hf_hub_cache() -> Path:
    ...
    try:
        ...
        # A Studio setting is live and must beat the process environment seeded
        # at startup. Environment/default installations still honour a scoped
        # live override, which is useful to callers and isolated tests.
...
class _SnapshotDownloadState:
    ...
    def _downloaded_bytes(self) -> Optional[int]:
        """Best-effort progress for exactly the selected snapshot files.

        Each file is counted across the sequential partial, finalized blob, and
        Windows snapshot-copy locations without counting an unrelated blob.
        """
        try:
