...
def _needs_nemotron_trust(model_name: str) -> bool:
    ...
    """
    mn = model_name.lower()
    return any(sub in mn for sub in _NEMOTRON_TRUST_SUBSTRINGS) and (
        mn.startswith("unsloth/") or mn.startswith("nvidia/")
    )
...
def _handle_load(backend, config: dict, resp_queue: Any) -> None:
    ...
    try:
        ...
        if not trust_remote_code and _needs_nemotron_trust(config["model_name"]):
