...
class TestHealthWaitMeasuresStalls:
    """Verify that timeout measures inactivity instead of total startup time."""

    # Grow RSS in 4 MiB steps, then idle.
    _WORKER = (
        "import sys, time\n"
        "held = []\n"
    ...
    # Page in mmap views and unmap them again, so RSS stays flat and nothing calls read().
    _MMAP_WORKER = (
    ...
    def test_mmap_page_ins_with_flat_resident_memory_count(self, monkeypatch, tmp_path):
        # Only page faults can show this load: rule out the CPU and byte signals.
        ...
        # Small writes: one large write can be cached as 2 MiB folios, one fault each.
    ...
    def test_cpu_after_an_unreadable_sample_is_measured_from_the_last_readable_one(
        ...
        # 20 ms over 0.9s is below the threshold despite the unreadable samples.
    ...
    def test_answering_health_probes_is_not_progress(self):
        # Measured on idle llama-server answering /health every 0.5s: at most 33 page
        # faults and 2% of a core per 5s window.
