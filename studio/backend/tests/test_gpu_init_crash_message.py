...
"""Tests for the backend-aware GPU-init crash message.

When llama-server hard-crashes on both the vision and the text-only attempt,
``load_model`` reports a GPU driver/runtime initialization crash. That message
used to hard-code ``ROCR_VISIBLE_DEVICES``, an AMD ROCm runtime variable, so an
Intel host running the Vulkan build was told to hide an AMD GPU it does not
have. ``_gpu_init_crash_message`` now picks its remediation from the ggml
backend libraries next to llama-server.
"""
...
class TestGpuInitCrashMessage:
    """The advice used to be hard-coded ROCm, which is how an Intel host was
    told to set ROCR_VISIBLE_DEVICES for an AMD GPU it does not have."""
    ...
    def test_unknown_backend_gives_neutral_advice(self, monkeypatch, backend):
        # None covers macOS too: ggml ships Metal as a .dylib the .so/.dll
        # matcher does not see, so it must resolve to neutral advice rather
        # than to another vendor's.
...
def test_the_caller_no_longer_hard_codes_rocm_advice():
    # The one caller sits deep inside load_model's vision retry ladder, which
    # has no seam a unit test can drive, so pin it structurally instead.
