...
/// Returns the managed studio root id, creating it when it does not exist.
///
/// Desktop-only installs never run the installer step that writes this file,
/// and every ownership check keys off it: without an id the app cannot claim
/// the backend it spawns, the backend never arms its parent watchdog, and a
/// relaunch after a force quit reports an unresolvable external conflict. So
/// the app writes the id itself instead of staying ownerless forever.
...
/// Startup self-heal for installs whose id was never written. Returns `Ok(None)`
/// when nothing is installed yet, so an empty machine is not given a studio root
/// for an install that does not exist; the spawn path creates the id instead.
...
fn ensure_studio_root_id_at_with_blank_observer(
    ...
    // Preserve the empty-machine behavior: setup checks must not create the
    // managed share directory solely to hold a lock when no install exists.
    ...
    // A blank file reads as missing but would still turn the no-clobber publish
    // below into a permanent failure, so drop it while holding the install-wide
    // lock. Re-reading under the lock prevents a stale blank observer from
    // deleting an id another desktop process has already published.
    ...
    // Another process created the file first. Its id is the one the backend
    // will report, so adopt the winner rather than overwriting it.
...
/// An id file that is empty or whitespace only is an interrupted write, not an
/// id: `install.sh` (`[ ! -s ]`) and `install.ps1` (`Length -gt 0`) replace one
/// too, and no backend can have read an id from it.
...
fn read_studio_root_id_file(path: &Path) -> Result<Option<String>, String> {
    ...
    // Any other unreadable content stays. A backend started while this file was
    // still valid keeps reporting the id it read then (the backend caches it at
    // import), so replacing the file would make that backend look foreign and
    // this app would start a second one beside it.
...
fn create_studio_root_id_file(path: &Path) -> Result<Option<String>, String> {
    ...
    // Name the temp file after the id it holds so two racing creators, in this
    // process or another, can never publish each other's half of the write.
...
/// Publishes `body` at `path` without clobbering a concurrent creator: the temp
/// file is written and flushed first, and hard-linking it into place is atomic
/// and fails when the destination already exists, so a reader can never see a
/// half-written id.
...
fn claim_private_file_with_link(
    ...
    match hard_link(tmp, path) {
        ...
        // The install-wide lock provides the no-clobber serialization here.
        // Rename the already flushed temp file so the fallback destination is
        // never observable with an empty or partial body.
...
/// Hands the backend its ownership identity. Unconditional by design: a backend
/// spawned without these disables its own parent watchdog and can never be
/// recognized as ours again.
...
mod tests {
    ...
    fn blank_studio_root_id_is_replaced_like_a_missing_one() {
        // An install interrupted between the id write and the rename leaves a
        // zero-length file behind; treating that as unreplaceable would block
        // every backend start until the user deleted it by hand.
        for blank in ["", "\n"] {
    ...
    fn stale_blank_observer_cannot_delete_a_competing_callers_id() {
        ...
        // The first caller has observed the blank file but has not removed it.
        // A second caller must wait instead of publishing an id that the stale
        // first caller could then delete.
