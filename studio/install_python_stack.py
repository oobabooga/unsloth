...
def _plan_pass(package_name: str, local_repo: str, ci_source_overlay: str) -> "dict | None":
    ...
    if manifest.get("no_torch") is not bool(NO_TORCH):
        return _refuse_evidence(f"no-torch mode moved ({manifest.get('no_torch')} -> {bool(NO_TORCH)})")
    if not NO_TORCH:
...
def install_python_stack() -> int:
    ...
    if NO_TORCH:
        ...
        if not _codec_ok:
            ...
            if _npp_major and _installed_distribution_version(f"nvidia-npp-cu{_npp_major}"):
                ...
                pass
            elif _npp_major and not pip_install_try(
