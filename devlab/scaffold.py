#!/usr/bin/env python3
"""Create a ready-to-push DevLab CI branch for a PR.
...
turns "a run I will lose in ten minutes" into "a message I read in one second".
  ...
  python devlab/scaffold.py --pr 9487 --out ci_pr9487 \\
      --tests studio/backend/tests/test_sd_cpp_install.py

  python devlab/scaffold.py --pr 9315 --merged --out ci_pr9315 --no-gpu \\
...
def main() -> int:
    ...
    ap.add_argument("--probe", default = "devlab/probes/pytest_probe.py")
    ap.add_argument("--criteria", default = "devlab/criteria/pytest_no_regression.py")
    ...
    branch = args.branch or f"amd-devlab-pr{args.pr}"
    out = args.out
