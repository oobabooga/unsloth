...
"""When the macOS loader probe may be answered from the marker instead of a spawn.

``preflight_macos_installed_binaries`` proves a bundle loads by running each installed
binary with ``--version``. On macOS the first such run against a newly written binary is
charged the OS's own first-launch assessment of the image and every dylib it links: 24 s
on an M1 mini, 44 s on a macos-15 runner, and it is paid on every update, including the
ones that install nothing.

The byte record #10648 added is written only by a run that had just loaded those exact
binaries, so a digest match answers the same question the spawn would. What the digests
cannot answer is whether the OS underneath moved, hence the ``host_profile`` half: it
carries ``macos_version``, and an OS move is the one way a loadable bundle stops loading
without a byte changing.

Both halves are tested: the skip, and every case that must still spawn -- no record (every
pre-#10648 install, which is what #9843 was written for), a changed macOS version, a digest
mismatch, and ``UNSLOTH_PREBUILT_FULL_CHECK``.
"""
...
# A sys.modules name of this suite's own: the sibling suites monkeypatch module globals,
# and a shared module would leak those across xdist workers.
...
def build_install(tmp_path: Path, host) -> Path:
    """A macOS install tree with a marker written by the real writer.

    Hand-written markers fail the fingerprint guard, which would make every case below
...
def count_spawns(monkeypatch) -> list[int]:
    """Count dyld load probes without running anything. The real probe spawns; this suite
    is about whether it is reached at all."""
    calls = [0]
...
def test_a_marker_without_a_byte_record_still_probes(tmp_path: Path, monkeypatch):
    """Every pre-#10648 install, and the shape #9843 was written for: nothing has hashed
    these binaries, so the only evidence available is dyld's answer."""
    host = macos_host()
    install_dir = build_install(tmp_path, host)
...
def test_a_macos_upgrade_probes_again(tmp_path: Path, monkeypatch):
    """The bundle loaded on 15.5; nothing says it loads on 26.0, and no byte moved."""
...
def test_a_replaced_binary_is_still_rejected(tmp_path: Path, monkeypatch):
    """The skip rests on the digests, so it must not survive them: a binary swapped under
    the marker fails the record and the install is reinstalled rather than reused."""
    host = macos_host()
    install_dir = build_install(tmp_path, host)
...
def test_a_bundle_that_cannot_load_is_rejected_when_it_is_probed(tmp_path: Path, monkeypatch):
    """#9843's guarantee, on the markers that still reach the probe."""
    host = macos_host()
    install_dir = build_install(tmp_path, host)
    marker = marker_of(install_dir)
    marker.pop("runtime_files")
