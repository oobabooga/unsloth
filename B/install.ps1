...
"@
    ...
    function New-UnslothTorchOverridesFile {
        ...
        # uv splits an --overrides value on spaces even when it arrives as one argument, so
        # C:\Users\John Doe\...\tmp1.tmp reached it as two missing files (#10722). The 8.3 name
        # is the same file in the same directory, so the caller's relative includes still resolve.
        if ($f.Contains(" ")) {
            $short = $null
            try { $short = (New-Object -ComObject Scripting.FileSystemObject).GetFile($f).ShortPath } catch { }
            if (-not $short -or $short.Contains(" ")) {
                # No usable short name: install as before the freeze existed rather than fail.
