...
def test_gpu_job_gets_the_tests() -> None:
    """The GPU job's pytest probe once got no --tests and exited at argparse in every state (PR 11871 run 36249647369)."""
    with tempfile.TemporaryDirectory() as td:
        out = Path(td) / "ci"
        r = subprocess.run([sys.executable, str(ROOT / "scaffold.py"), "--pr", "1", "--out", str(out),
                            "--branch", "b", "--spoof-devices", "1", "--tests", "tests/test_x.py"],
                           capture_output = True, text = True, encoding = "utf-8")
        check("scaffolding a GPU job succeeds", r.returncode == 0, r.stdout + r.stderr)
        wf = out / ".github" / "workflows" / "b.yml"
        text = wf.read_text(encoding = "utf-8") if wf.is_file() else ""
        gpu = text.split("out/gpu", 1)[1][:400] if "out/gpu" in text else ""
        check("the GPU Differential passes --tests to the pytest probe", "--tests tests/test_x.py" in gpu, gpu)
...
def test_scaffold_windows_emits_the_measured_selector() -> None:
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        # The workflow calling a preamble the tree does not ship died at step 1 (PR 11871 run 36249644094).
        check("the scaffolded tree ships lib/preamble.ps1",
