...
def test_every_file_a_scaffold_references_is_shipped() -> None:
    """A template naming a file the scaffold does not copy fails at that step.

    Live: lib/preamble.ps1 was named by templates/workflow_windows.yml, by the
    README and by the lint rules, and did not exist. Every Windows scaffold ever
    produced died on its FIRST step with CommandNotFoundException, and this
    selftest did not catch it because the only assertion was that the template
    TEXT mentioned preamble.ps1, which it did. Asserting a file's name is not
    asserting the file.
    """
    print("\nEvery amd_ci path a generated workflow names is really shipped")
    for windows in (False, True):
        with tempfile.TemporaryDirectory() as td:
            out = Path(td) / "ci"
            args = [sys.executable, str(ROOT / "scaffold.py"), "--pr", "1",
                    "--out", str(out), "--branch", "b", "--tests", "tests/test_x.py"]
            if windows:
                args.append("--windows")
            r = subprocess.run(args, capture_output = True, text = True)
            label = "windows" if windows else "linux"
            check(f"{label} scaffolding succeeds", r.returncode == 0, r.stdout + r.stderr)
            wf = out / ".github" / "workflows" / "b.yml"
            text = wf.read_text(encoding = "utf-8") if wf.is_file() else ""
            # Both separators: the Windows template writes .\amd_ci\lib\... .
            # rstrip the sentence punctuation: these names also appear in prose
            # comments, where a trailing full stop is not part of the path.
            named = {n.rstrip(".,;:)")
                     for n in re.findall(r"amd_ci[\\/][A-Za-z0-9_\\/.-]+", text)}
            missing = sorted(
                n for n in named
                if not (out / n.replace("\\", "/")).is_file()
                and not (out / n.replace("\\", "/")).is_dir()
            )
            check(f"{label}: every amd_ci path in the workflow exists in the scaffold",
...
def main() -> int:
    ...
    test_every_file_a_scaffold_references_is_shipped()
