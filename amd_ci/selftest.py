...
def test_kfd_gfx_target_decoding() -> None:
    print("\nOn Linux the gfx target is read from the KFD without torch")
    cap = _load(ROOT / "lib" / "capability.py")
    # 110501, off this pool's own gfx_target_version. The README warns the value
    # is NOT 110511, and a decoder that spelled the step in decimal would produce
    # gfx11511 here, so this is the assertion that keeps both mistakes out.
    check("110501 decodes to gfx1151",
          cap.gfx_target_from_kfd_version(110501) == "gfx1151",
          cap.gfx_target_from_kfd_version(110501))
    # The step is a hex digit; that is where gfx90a's name comes from.
    check("90010 decodes to gfx90a",
          cap.gfx_target_from_kfd_version(90010) == "gfx90a",
          cap.gfx_target_from_kfd_version(90010))
    check("120001 decodes to gfx1201",
          cap.gfx_target_from_kfd_version(120001) == "gfx1201",
          cap.gfx_target_from_kfd_version(120001))
    # Every CPU node reports 0, and a CPU is not a GPU.
    check("a CPU node (0) decodes to nothing", cap.gfx_target_from_kfd_version(0) == "")
    if sys.platform != "linux":
        check("the KFD read is a no-op off Linux", cap.linux_kfd_gfx_targets() == [])
    ...
    for path in [*(ROOT / "lib").glob("*.py"), *(ROOT / "probes").glob("*.py"),
        ...
        for node in ast.walk(ast.parse(path.read_text(encoding = "utf-8"))):
            ...
            if any(k.arg == "encoding" for k in node.keywords):
                continue
            if name == "open":
                # `tarfile.open` / `gzip.open` are never text I/O, and their mode
                # strings ("r:gz") have no "b" to recognise them by. Judged on the
                # module they are called on, so a bare `open(...)` is still caught.
                owner = getattr(getattr(f, "value", None), "id", "")
                if owner in ("tarfile", "gzip", "zipfile", "lzma", "bz2", "os"):
                    continue
