...
| E103 | branching on `RUNNER_OS` in a Windows job, where it is EMPTY |
...
Measured today, on four distinct machines (`X2-LA01-S05-D01`, `X2-LA01-S04-D02`,
`DESKTOP-O81NVEC`, `DESKTOP-52OM6HM`), all identical:
...
- `shell: bash` gives `bash: command not found`. One box has a WindowsApps
  ...
  job cannot see.
- **`$env:RUNNER_OS` is EMPTY** on these jobs. Do not branch on it.
