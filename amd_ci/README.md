...
### Prebuilt releases, no PR and no source

For "does the shipped binary do this on Strix Halo": two release tags of
`unslothai/llama.cpp`, one model, one environment, same VOID-unless-base-shows-it
rule. Nothing is compiled on the runner.
...
python amd_ci/scaffold.py --prebuilt b10715-mix-86bd2d3,b10798-mix-659e406 \
    ...
    --out ci_uma            # --windows for the Windows zips on the Windows boxes
```

The generated workflow gates on gfx1151 without torch (`rocminfo` on Linux, the
video controller name on Windows), fetches each release archive once
(`lib/fetch_llamacpp.py`), a small sentinel model, wikitext prompts and the
model's shards at a pinned revision (`lib/fetch_gguf.py`), writes `states.json`
from the two binary directories (`lib/write_states.py`), runs `differential.py`
with `probes/llamacpp_server_probe.py` and `criteria/llamacpp_server_clean.py`,
then each control arm, and prints one table over every arm
(`lib/summarize_arms.py`).

The probe drives `llama-server` over HTTP at temperature 0 through three cells:
`single` (the negative control, clean in every state or the harness is broken),
`multiseg` (multi-segment prompts) and `unified4` (four concurrent long prompts
on `-np 4 --kv-unified -b 2048 -ub 512`). It records corruption signatures,
server-log faults, cross-slot text sharing and the binary's fingerprint, and
runs a sentinel model before and after each state so a poisoned GPU reads as
INCONCLUSIVE. The criteria module judges; the probe does not.

Control arms differ from the differential by one variable each: `--control-env
absent` unsets the `--env` variables, `--control-env K=V` overrides one,
`--control-asset` is another backend, `--control-tag` an older build. They are
read against the verdict, not folded into it. Pass the condition under test as
`--env` and its absence as a control: the base must show the defect under that
condition or the run is VOID.

`--model REPO:FOLDER` resolves the folder's `.gguf` files and pins the current
commit; `REPO@SHA:FILE,FILE` scaffolds offline.
...
| E107 | a `strix-halo` selector carrying `ephemeral`, a label no runner has; the job never starts and holds the GPU group |
...
- **The system Python has no torch.** When torch reports nothing, `capability.py`
  reads the arch and device count from `rocminfo` (Linux) or
  `Win32_VideoController` (Windows), so `gate.py --no-torch --expect-arch
  gfx1151` works from either. `torch_detection` stays false.
...
- **`pending` waits for the concurrency group, `queued` for a runner.** A run
  on a label no runner carries (`ephemeral`) sits `queued` forever and holds the
  group, so everything behind it is `pending`; from the run list that looks like
  a busy pool. `gh run list --status queued` on the host repo finds it. When only
  the harness was wrong, `lib/redecide.py` re-derives a verdict from a finished
  run's `observations.json` instead of spending a second run.
- **Name the encoding on every text read and write.** `Path.read_text()`
  defaults to cp1252 on Windows; a UTF-8 prompt read that way is mojibake, and
  the model answered it with a single EOS that read as a defect. A selftest
  scans the toolkit for unqualified text I/O.
