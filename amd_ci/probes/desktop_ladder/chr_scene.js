...
(() => {
  ...
  W.run = async (opts) => {
    const o = Object.assign({
      ...
      scrollStepPx: 900, scrollMaxSteps: 400, scrollPasses: 1, scrollBudgetMs: 180000,
    ...
    try {
      ...
      mark("scroll");
      // ── THE GESTURE, CORRECTED. What amdv_scene.js does here does not scroll in Chromium ──
      //
      // The original is `el.scrollTop = el.scrollTop - 900` ten times, 80 ms apart. The thread
      // viewport carries `scroll-smooth` (thread.tsx:1761), and under `scroll-behavior: smooth`
      // a scrollTop ASSIGNMENT animates rather than jumps. Eighty milliseconds later the read of
      // `el.scrollTop` returns a position that has barely moved, so the next write aims 900 px
      // below THAT, and the up-passes and down-passes cancel.
      //
      // Measured in headless Chromium at r500K, corpus 23cd2464: 54,000 px commanded, 6,610 px
      // actually travelled, and the viewport never got further than 1,107 px from the bottom of
      // a 316,829 px thread. That window is a jiggle, not a traversal, and it reads 58 fps
      // because almost nothing happens in it. `scroll_detail` cannot show this: it records the
      // first and last position only, and both are the bottom either way.
      //
      // So this phase uses the campaign's own established gesture instead
      // (scripts/trav_probe.py::SEGMENT_JS, itself studiobench's `scroll_traverse`):
      //   * `scrollTo({ behavior: "instant" })`, which is not subject to `scroll-smooth`;
      //   * a real WheelEvent, so the app's intent-aware autoscroll reads a user scroll rather
      //     than a programmatic one and detaches from follow mode as it would for a person;
      //   * one step per PAINTED frame, so a slow engine takes longer rather than skipping
      //     travel -- distance is held fixed across arms and TIME is what varies;
      //   * commanded and travelled pixels recorded, so "did it scroll" is answered by the
      //     payload and never assumed again.
      const scrollT0 = performance.now();
      const el = scrollerAt(window.innerWidth / 2, window.innerHeight * 0.5);
      let scrollDetail = "no scroller";
      const scrollTrace = { commanded_px: 0, travelled_px: 0, steps: 0, passes: 0,
                            reached_top: false, deadline_hit: false, step_ms: [] };
      if (el) {
        const h0 = el.scrollHeight, top0 = el.scrollTop;
        const stepPx = o.scrollStepPx, maxSteps = o.scrollMaxSteps;
        const nextPaint = () => new Promise((r) => nativeRaf(() => nativeRaf(r)));
        const deadline = performance.now() + o.scrollBudgetMs;
        const leg = async (dir) => {
          for (let i = 0; i < maxSteps; i++) {
            if (performance.now() > deadline) { scrollTrace.deadline_hit = true; return; }
            const from = el.scrollTop;
            const limit = el.scrollHeight - el.clientHeight;
            const target = dir < 0 ? Math.max(0, from - stepPx)
                                   : Math.min(limit, from + stepPx);
            if (target === from) { if (dir < 0) scrollTrace.reached_top = true; return; }
            scrollTrace.commanded_px += Math.abs(target - from);
            const s0 = performance.now();
            el.dispatchEvent(new WheelEvent("wheel", { deltaY: dir * stepPx,
                                                       bubbles: true, cancelable: true }));
            el.scrollTo({ top: target, behavior: "instant" });
            await nextPaint();
            scrollTrace.travelled_px += Math.abs(el.scrollTop - from);
            scrollTrace.steps += 1;
            scrollTrace.step_ms.push(Math.round((performance.now() - s0) * 10) / 10);
          }
        };
        for (let pass = 0; pass < o.scrollPasses; pass++) {
          scrollTrace.passes += 1;
          await leg(-1);
          await sleep(300);
          await leg(+1);
          await sleep(300);
          if (scrollTrace.deadline_hit) break;
        }
        el.scrollTo({ top: el.scrollHeight, behavior: "instant" });
        scrollDetail = `${el.tagName}.${(el.className || "").toString().slice(0, 30)} ` +
                       `h=${h0} top ${top0}->${el.scrollTop} ` +
                       `commanded=${scrollTrace.commanded_px} travelled=${scrollTrace.travelled_px} ` +
                       `steps=${scrollTrace.steps}`;
      }
      const scrollEl = performance.now() - scrollT0;
      W.scrollTrace = scrollTrace;
      const scrollS = slice();
      ...
      if (o.skipSend) {
        ...
        post({
          ...
          clamp, scroll_detail: scrollDetail, scroll_trace: scrollTrace,
