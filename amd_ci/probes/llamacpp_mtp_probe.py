...
# `-sm tensor` aggregates several devices into one Meta device, so the per-layer
# lines stop naming the real devices. These two recover the membership.
TP_META_RE = re.compile(
    r"creating a Meta device for tensor parallelism from (\d+) devices")
TP_DEV_RE = re.compile(
    r"llama_prepare_model_devices:\s+- device \d+:\s+(\S+)")
...
def main() -> int:
    ...
    # Tensor parallelism does not show up in layers_by_device, and reading that
    # field alone gets the answer exactly backwards. Under `-sm tensor`
    # llama.cpp builds ONE aggregate device and assigns every layer to it:
    #   llama_prepare_model_devices: creating a Meta device for tensor
    #   parallelism from 3 devices:
    #     - device 0: ROCm0 (AMD Radeon Graphics (dev p0/v0))
    # so the per-layer lines all read `Meta()` and a device-counting gate sees
    # ONE device and calls a working three-way tensor split a single-device run.
    # That misread cost a run here. The membership list is the ground truth.
    m = TP_META_RE.search(text)
    obs["tp_device_count"] = int(m.group(1)) if m else None
    obs["tp_devices"] = TP_DEV_RE.findall(text)
    obs["tensor_parallel"] = bool(obs["tp_device_count"] and obs["tp_device_count"] >= 2)
