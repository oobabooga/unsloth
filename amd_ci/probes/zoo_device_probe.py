...
def run_child(leg: str, args, work: Path, timeout: int) -> dict:
    ...
    # Per state, never inherited: a cache compiled by the base zoo must not serve the head.
    env["UNSLOTH_COMPILE_LOCATION"] = str(work / "unsloth_compiled_cache")
    env["UNSLOTH_DISABLE_AUTO_UPDATES"] = "1"
    # On the runner nothing may land outside $RUNNER_TEMP; the defaults for these are
    # under $HOME. Shared across states (one model download), not across jobs.
    root = os.environ.get("AMD_CI_WORK") or os.environ.get("RUNNER_TEMP")
    if root:
        env.setdefault("HF_HOME", os.path.join(root, "hf_home"))
        env.setdefault("XDG_CACHE_HOME", os.path.join(root, "xdg_cache"))
