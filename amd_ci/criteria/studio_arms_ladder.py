...
A REGRESSION IS LOOKED FOR IN TWO PLACES, BECAUSE ONE BAR ON THE MEAN CANNOT SEE A TAIL. Run
`33040070879` carried a 100K scroll cell whose effective frame rate moved 58.5, 57.6 -> 54.5, 53.8
(7% on the mean, repetitions not overlapping) while its WORST FRAME moved 49 ms -> 440 ms, nine
times longer, in both repetitions. A single 20% bar on the mean printed "no cell has head worse
than pre" over the top of a visible hitch that the pre-campaign build did not have. The mean and
the tail are different failures with different mechanisms -- a slower steady state against one
lump of deferred work landing in one frame -- so each gets its own bar here, either of which alone
is enough to name a cell as regressed:

  MEAN  effective fps down by at least `REGRESSION_MIN_LOSS`, repetitions not overlapping.
  TAIL  worst frame at least `TAIL_MIN_RATIO` times the pre cell's AND at least
        `TAIL_MIN_WORST_MS` in absolute terms, repetitions not overlapping.

The absolute floor is what keeps the tail bar quiet: 8 ms -> 24 ms is 3x and is not a hitch, so a
ratio alone would fire on noise at 60 fps. 100 ms is the same threshold the report already uses
for `frames_over_100_pct`, and it is roughly six frames at 60 Hz, i.e. the point at which a scroll
stops looking continuous.
"""
...
# A head cell counts as a regression when its mean effective frame rate is this much worse than
# the same pre cell AND the repetitions agree about the direction. It was 0.20, which is the bar
# a 7% loss with a 9x worse tail passed under in run 33040070879. The repetitions-must-not-overlap
# requirement is what carries the confidence here, not the size of the number: two repetitions per
# arm that do not overlap at all are a settled direction, and on this venue a cell's repetitions
# sit within about 1% of each other (58.5/57.6, 54.5/53.8), so 5% is several spreads out.
...
# The tail bar, which is independent of the mean and catches what the mean cannot: one lump of
# work landing in one frame. Both conditions, and the repetitions must not overlap.
TAIL_MIN_RATIO = 2.0
TAIL_MIN_WORST_MS = 100.0
...
def _worst_values(obs: dict, arm: str, rung: str, phase: str) -> list[float]:
    """The longest rAF gap in each repetition of a cell, i.e. the hitch a user would have seen."""
    out = []
    for r in _cells(obs, arm, rung):
        v = _sub(_phase(r, phase), "raf").get("max_ms")
        if isinstance(v, (int, float)):
            out.append(float(v))
    return out


def _action_worst_values(obs: dict, arm: str, rung: str, name: str) -> list[float]:
    out = []
    for r in _cells(obs, arm, rung):
        v = _sub(_action(r, name), "raf").get("max_ms")
        if isinstance(v, (int, float)):
            out.append(float(v))
    return out
...
def _cell_regression(label: str, pre: list[float], head: list[float],
                     pre_worst: list[float], head_worst: list[float]) -> str | None:
    """One cell judged on TWO bars, the mean and the tail, either of which alone is enough.

    Returns a line for the report, or None. A line that names a SETTLED regression never carries
    the word OVERLAP, because the verdict filters settled from unsettled on exactly that word.
    """
    settled: list[str] = []
    overlapping: list[str] = []

    mp, mh = _mean(pre), _mean(head)
    if mp is not None and mh is not None and pre and head and mp >= REGRESSION_MIN_ABS_FPS:
        loss = 1 - (mh / mp)
        if loss >= REGRESSION_MIN_LOSS:
            txt = (f"head {_reps_str(head)} against pre {_reps_str(pre)} fps, "
                   f"{loss * 100:.0f}% WORSE on the mean")
            # The repetitions must agree about the direction, or this is one slow draw.
            (settled if max(head) < min(pre) else overlapping).append(txt)

    wp, wh = _mean(pre_worst), _mean(head_worst)
    if wp and wh and pre_worst and head_worst and wp > 0 \
            and wh >= TAIL_MIN_WORST_MS and (wh / wp) >= TAIL_MIN_RATIO:
        txt = (f"worst frame {_reps_str(head_worst, 0)} ms against pre "
               f"{_reps_str(pre_worst, 0)} ms, {wh / wp:.1f}x WORSE in the tail")
        (settled if min(head_worst) > max(pre_worst) else overlapping).append(txt)

    if settled:
        return f"{label}: " + "; ".join(settled)
    if overlapping:
        return (f"{label}: " + "; ".join(overlapping)
                + ", but the repetitions OVERLAP, so this is not settled")
    return None
...
def _regressions(obs: dict) -> list[str]:
    ...
    for rung in _rungs(obs):
        for phase in SCORED_PHASES:
            line = _cell_regression(
                f"{rung} {phase}",
                _fps_values(obs, "pre", rung, phase), _fps_values(obs, "head", rung, phase),
                _worst_values(obs, "pre", rung, phase), _worst_values(obs, "head", rung, phase))
            if line:
                out.append(line)
    for name in ACTIONS:
        for rung in _rungs(obs):
            ...
            line = _cell_regression(
                f"{rung} action:{name}",
                [x for x in pre if isinstance(x, (int, float))],
                [x for x in head if isinstance(x, (int, float))],
                _action_worst_values(obs, "pre", rung, name),
                _action_worst_values(obs, "head", rung, name))
            if line:
                out.append(line)
    return out
...
def table(obs: dict) -> str:
    ...
    lines.append("")
    if regs:
        lines.append("`head` is worse than `pre` in these cells, on the mean, in the tail, or "
                     "both:")
        lines.append("")
        for r in regs:
            lines.append(f"- {r}")
    else:
        lines.append(f"No cell has `head` worse than `pre`, at any rung, in any of "
                     f"{', '.join(SCORED_PHASES)} or "
                     f"{', '.join('action:' + a for a in ACTIONS)}, on either bar: not by "
                     f"{REGRESSION_MIN_LOSS * 100:.0f}% or more on the mean effective frame "
                     f"rate, and not by {TAIL_MIN_RATIO:g}x or more on the worst frame with that "
                     f"worst frame at least {TAIL_MIN_WORST_MS:g} ms, in either case with the "
                     f"repetitions agreeing about the direction.")
    lines.append("")
    ...
    for rung in _rungs(obs):
        for name in ACTIONS:
            ...
            for arm in ARMS:
                ...
                # AN ACTION THAT DID NOT APPLY HAS NO FRAME RATE, and it must not be printed as
                # one. A not-applicable action returns immediately, so its window is 0 or 1 ms
                # with zero frames, and `1000 * 0 / 1` is 0.0: a real number, in the fps column,
                # for a thing that never ran. Observed in a live run as "0.0 fps" for the head
                # arm at 0K beside "not applicable" for the pre arm, which is the same non-event
                # rendered two ways, one of which reads as a total collapse.
                fps = [_action(r, name).get("eff_fps") for r in cells
                       if not _action(r, name).get("not_applicable")]
...
def verdict(obs: dict) -> tuple[str, str]:
    ...
    if head_fps >= FIXED_MIN_FPS and ratio and ratio >= FIXED_MIN_RATIO:
        # A cell that got WORSE does not stop being worse because the top rung got better, and a
        # verdict string is what gets quoted. It is therefore in the string rather than only in
        # the table underneath it.
        return ("CONFIRMED_WITH_REGRESSION" if settled_regs else "CONFIRMED"), (
