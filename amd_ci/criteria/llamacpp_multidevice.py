...
def gates(obs: dict) -> list[tuple[str, bool, str]]:
    ...
    # Two ways to be genuinely multi-device, and only counting the first one
    # gets `-sm tensor` exactly backwards. Layer/row splits name the real
    # devices per layer; a TENSOR split builds one aggregate `Meta()` device and
    # assigns every layer to it, so the per-layer count reads 1 for a working
    # three-way split. The probe recovers the membership from llama.cpp's own
    # "creating a Meta device for tensor parallelism from N devices" line.
    span = {n: max(len(split[n]), v.get("tp_device_count") or 0)
            for n, v in states.items()}
    really_split = all(s >= MIN_DEVICES for s in span.values())
    out.append((f"every state spanned >= {MIN_DEVICES} non-CPU devices", really_split,
                "; ".join(
                    f"{n}=" + (f"tensor-parallel over {v.get('tp_devices')}"
                               if v.get("tensor_parallel") else str(split[n]))
                    for n, v in states.items())))

    # A split that put one layer on the second device is technically two devices
    # and practically one. Ask for a real share -- but only where per-device
    # layer counts mean anything. Under tensor parallelism every layer sits on
    # the Meta device by construction, so this check would compare a number
    # against itself; the membership gate above is what carries that case.
    per_layer = {n: d for n, d in split.items()
                 if not (states[n].get("tensor_parallel"))}
    if per_layer:
        balanced = {}
