...
def main() -> int:
    ...
    # Absolute, always. `git worktree add` runs with cwd inside the clone, so a
    # RELATIVE --root puts every worktree under <root>/repo/<root>/states/... and
    # the printed paths then name directories that do not exist. CI passes an
    # absolute $AMD_CI_WORK and never saw it; a local run does.
