...
step runs, or worse, succeeds while measuring nothing:
  ...
  W103  Branching on `$env:RUNNER_OS` in a Windows job. It has been observed
        blank on these runners, though it read correctly on the one measured
        most recently; see the rule text.
  ...
  E107  A strix-halo selector carrying `ephemeral`, a label no runner has had
        since the pool became `devlab-dispatch`. The job never starts, sits
        `queued`, and holds the shared GPU group, so every run behind it is
        `pending` until someone cancels it.
...
def lint_windows(where: str, body: str, shell: str | None) -> list[Finding]:
    ...
    if re.search(r"RUNNER_OS", body):
        out.append(("W103", where,
                    "branches on `RUNNER_OS`. An earlier session recorded it EMPTY on four of "
                    "the Windows boxes; on DESKTOP-NILQUD2 (runner 2.337.0) it read 'Windows' "
                    "correctly. The likely explanation for the earlier reading is the E104 "
                    "trap, `${RUNNER_OS:-?}` being bash syntax inside a PowerShell block. "
                    "Either way it has been observed blank, so echo it before you trust it, "
                    "and prefer branching on the job over branching on the environment"))
...
def lint_workflow(path: Path) -> list[Finding]:
    ...
    for job_name, job in (doc.get("jobs") or {}).items():
        ...
        if "self-hosted" in labels and "strix-halo" in labels:
            ...
            if "ephemeral" in labels:
                findings.append(("E107", job_name,
                                 "selects the strix-halo pool with the `ephemeral` label, which no "
                                 "runner carries; the job never starts and holds the shared GPU "
                                 "group. Use `devlab-dispatch`"))
