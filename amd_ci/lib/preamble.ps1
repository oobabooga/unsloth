...
#   $env:RUNNER_OS      has been seen EMPTY on these jobs, and read 'Windows'
#                       correctly on DESKTOP-NILQUD2 under runner 2.337.0. The
#                       likely cause of the blank reading is bash `${RUNNER_OS:-?}`
#                       syntax inside a PowerShell block, which expands to "".
#                       It is echoed below every run rather than trusted.
...
# Printed every run rather than trusted, because it has been observed blank here.
...
Write-Host "RUNNER_OS reads '$env:RUNNER_OS' (observed blank on some of these runners)"
