#!/usr/bin/env python3
"""Fetch one GGUF file from the Hugging Face Hub, resuming on failure.

stdlib only. The revision is required so a repo update cannot silently change
the file; a dropped connection resumes from the bytes on disk.

  python amd_ci/lib/fetch_gguf.py --repo unsloth/Qwen3-0.6B-GGUF \\
      --file Qwen3-0.6B-Q4_K_M.gguf --revision <sha> --as-name sentinel.gguf \\
      --local-dir "$AMD_CI_WORK/models/small" --out "$AMD_CI_WORK/out/model_small.json"

Exits 1 unless the file is complete.
"""
    ...
    for attempt in range(attempts):
        ...
        try:
            with _open(url, start, token) as r:
                ...
                with (dest.open(mode = "ab") if start else dest.open(mode = "wb")) as fh:
                    while True:
                        buf = r.read(CHUNK)
                        if not buf:
                            break
...
def resolve_url(repo: str, revision: str, file: str) -> str:
    """The Hub's resolve URL. A folder prefix in `file` stays a folder prefix."""
    return f"https://huggingface.co/{repo}/resolve/{revision}/{file.lstrip('/')}"
...
def main() -> int:
    ...
    url = resolve_url(args.repo, args.revision, args.file)
