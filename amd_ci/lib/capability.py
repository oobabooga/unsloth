...
KNOWN_GAPS = {
    ...
    "studio_production_bundle": (
        "needs Studio installed and its frontend built by install.sh, i.e. an `npm run build` "
        "dist. A vite dev server is not a substitute: React's dev build inflates the axis "
        "under investigation by about 3.2x"
    ),
    "studiobench_ladder": (
        "needs the studiobench frozen corpus and Seeder to have put a thread of the declared "
        "size into a running Studio, at two or more rungs. A rung that failed to seed is not a "
        "smaller rung, it is a different experiment"
    ),
    # Desktop (Tauri). Deliberately separate names from the web UI ones: Studio in a browser
    # and Studio in the Tauri shell are the same frontend in two different processes, and only
    # one of them runs studio/src-tauri/src/linux_webkit.rs.
    "rust_toolchain": "needs cargo and rustc on the host; nothing here installs them",
    "tauri_build_deps": (
        "needs the Tauri v2 Linux BUILD closure resolvable through pkg-config "
        "(webkit2gtk-4.1, javascriptcoregtk-4.1, libsoup-3.0, gtk+-3.0). Established by "
        "compiling and LINKING against it, never by a pkg-config --modversion string, which "
        "succeeds on a .pc file with no headers behind it"
    ),
    "desktop_tauri_app": (
        "needs the Tauri binary to have realised a window and PAINTED the Studio frontend in "
        "it. A process that started is not an app that functions"
    ),
    "desktop_gpu_compositing": (
        "needs the Tauri app's own web process observed compositing on the GPU, by amdgpu "
        "per-process fdinfo accounting against a software negative control. WebKitGTK "
        "hardcodes `Apple GPU` in WEBGL_debug_renderer_info on Linux/AMD, so no in-page "
        "string can establish this"
    ),
    "windows_webview2": (
        "needs Windows; Desktop there renders through WebView2 (Chromium), a different engine "
        "with a different compositor from WebKitGTK. Wine cannot host it faithfully"
    ),
    "macos": "needs a macOS host; this runner is Linux",
    "macos_wkwebview": (
        "needs macOS; Desktop there renders through WKWebView on the WebKit Cocoa port, "
        "which shares WebCore with WebKitGTK but not the compositor or the display link"
    ),
    "wayland_session": (
        "needs a live Wayland compositor. This runner drives Xvfb, so the Wayland arm of "
        "linux_webkit.rs (which applies WEBKIT_DMABUF_RENDERER_FORCE_SHM) is never taken here"
    ),
}
...
PROBE_ESTABLISHED = frozenset({
    ...
    "studio_production_bundle", "studiobench_ladder",
    "tauri_build_deps", "desktop_tauri_app", "desktop_gpu_compositing",
})
...
def detect(require_torch: bool = True, observed: dict[str, bool] | None = None) -> HostProfile:
    ...
    p.capabilities = {
        ...
        "studio_production_bundle": False,
        "studiobench_ladder": False,
        # Desktop (Tauri).
        "rust_toolchain": bool(shutil.which("cargo") and shutil.which("rustc")),
        "tauri_build_deps": False,
        "desktop_tauri_app": False,
        "desktop_gpu_compositing": False,
        "windows_webview2": p.system == "Windows",
        "macos": p.system == "Darwin",
        "macos_wkwebview": p.system == "Darwin",
