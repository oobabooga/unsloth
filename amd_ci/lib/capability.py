...
KNOWN_GAPS = {
    ...
    "no_bf16_gpu": (
        "needs a GPU WITHOUT native bfloat16: RDNA1/2 (gfx101x, gfx103x) or a "
        "pre-Ampere NVIDIA card such as the T4. This host's GPU HAS bf16, so any "
        "defect whose visible form depends on bf16 being unusable (Triton failing "
        "to lower it on gfx10, and the LLVM abort that follows) cannot be "
        "reproduced here. Only the dtype fault underneath it can"
    ),
...
class HostProfile:
    ...
    # torch.cuda.is_bf16_supported(), or None when there was no device to ask.
    # None is "unmeasured", which capabilities_for keeps distinct from False.
    bf16_supported: Any = None
...
def detect(require_torch: bool = True) -> HostProfile:
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            try:
                p.bf16_supported = bool(torch.cuda.is_bf16_supported())
            except Exception:  # noqa: BLE001
                p.bf16_supported = None
...
def capabilities_for(p: "HostProfile") -> dict[str, bool]:
    ...
    return {
        ...
        # A GPU that CANNOT do bf16 is itself a capability: it is what turns the
        # dtype fault in unsloth-zoo 1316 into an unrecoverable LLVM abort. `None`
        # means nothing was asked, which must not read as "this host qualifies".
        "no_bf16_gpu": real_gpu_count > 0 and p.bf16_supported is False,
