...
KNOWN_GAPS = {
    "windows": (
        "needs a Windows host; THIS job ran on Linux. Not a property of the pool: "
        "devlab-dispatch also carries four Windows 11 Strix Halo boxes, and "
        "`runs-on: [self-hosted, Windows, strix-halo, devlab-dispatch]` with "
        "`shell: powershell` matches them in seconds. Re-run the job there rather "
        "than reporting this as unreachable"
    ),
    "windows_rocm_wddm": (
        "needs Windows ROCm. `rocm_windows_free_is_untrusted()` is "
        "`sys.platform == 'win32' and IS_ROCM`, so on Linux the WDDM free cap is "
        "never applied and `trusted_mem_get_info` is a pure passthrough. The "
        "Windows runners themselves ARE reachable now (see `windows`); what is "
        "unmeasured is whether a ROCm-enabled torch is installed on them, so a "
        "job that wants this must `--require rocm` on Windows and let the gate FAIL"
    ),
    "docker": "needs a docker CLI on PATH; this host has none",
    "windows_docker": (
        "needs Docker on Windows. Measured on all four Windows AMD runners "
        "(X2-LA01-S05-D01, X2-LA01-S04-D02, DESKTOP-O81NVEC, DESKTOP-52OM6HM): "
        "`docker` is not installed, `docker : The term 'docker' is not recognized`. "
        "Windows on this pool is reachable for NON-Docker workloads only; anything "
        "containerised has to stay on the Linux runners"
    ),
    ...
    "gfx1201_rdna4": (
        "needs an RDNA4 gfx1201 card (RX 9070 XT). The pool is gfx1151 (Strix Halo, "
        "Radeon 8060S), so a result here bounds the LOGIC only: whether the generic "
        "ROCm sd.cpp prebuilt carries kernels for gfx1201 is a different question "
        "about a different code-object list, and this host cannot answer it"
    ),
    "gfx1100_rdna3": (
        "needs an RDNA3 gfx1100 card (RX 7900 XTX). The pool is gfx1151, so the "
        "tensor-loading stall #8814 reports on that card is not reproducible here"
    ),
    "rocm_runtime_libs": (
        "needs the ROCm 7.x runtime sonames the upstream sd.cpp ROCm archive lists "
        "as NEEDED while bundling none of them (libamdhip64.so.7, libhipblas.so.3, "
        "librocblas.so.5) to be resolvable by the dynamic loader. Read from "
        "`ldconfig -p`, which is a Linux question: on Windows it is UNDETERMINED "
        "rather than absent, and there the same dependency appears as amdhip64.dll "
        "/ hipblas.dll / rocblas.dll on PATH"
    ),
    "no_rocm_runtime_host": (
        "needs an AMD machine that has a GPU but NO ROCm userspace, which is the "
        "population a ROCm -> Vulkan fallback exists to serve. Every Linux box in "
        "this pool is a ROCm development host, so the generic ROCm prebuilt finds "
        "the runtime it needs here. That a working host keeps working is measurable "
        "here; that a runtime-less host is rescued by the Vulkan rung is NOT, and no "
        "amount of green on this pool can stand in for it"
    ),
    "vulkan": (
        "needs an enumerable Vulkan ICD. This is read from /usr/share/vulkan/icd.d, "
        "which is a Linux path: on a Windows job it is UNDETERMINED rather than absent"
    ),
...
class HostProfile:
    ...
    # Read once in detect(), so capabilities_for() stays a pure function of the
    # profile and selftest can exercise the Windows/Docker rule on a host whose
    # own docker presence is irrelevant to the runner being described.
    docker_cli: bool = False
    # Win32_VideoController names, read when torch reported no device on Windows.
    windows_video_controllers: list[str] = field(default_factory = list)
    # Which of the ROCm runtime sonames the upstream sd.cpp ROCm prebuilt NEEDS can
    # be resolved by the dynamic loader on this host. None off Linux, where ldconfig
    # does not exist and the answer is UNDETERMINED rather than absent. Read in
    # detect() so capabilities_for() stays a pure function of the profile.
    rocm_runtime_sonames: dict[str, bool] | None = None

    def to_json(self) -> str:
        return json.dumps(asdict(self), indent = 2)


# Windows has no rocminfo, so without a ROCm torch the video controller name is
# the only arch signal. Only parts this pool carries; unknown names map to nothing.
WINDOWS_VIDEO_CONTROLLER_ARCHS = {
    "8060S": "gfx1151",  # Radeon 8060S, Strix Halo
    "8050S": "gfx1151",  # Radeon 8050S, Strix Halo
}


def archs_from_windows_video_controllers(names: list[str]) -> list[str]:
    """Map Win32_VideoController names to gfx targets, dropping the unknown."""
    out: list[str] = []
    for name in names:
        for part, arch in WINDOWS_VIDEO_CONTROLLER_ARCHS.items():
            if part in name:
                out.append(arch)
                break
    return out
...
def windows_video_controllers() -> list[str]:
    """GPU names as Windows reports them, or [] anywhere that is not Windows."""
...
def detect(require_torch: bool = True) -> HostProfile:
    ...
    # Arch from the video controller when torch saw nothing. gpu_count stays 0:
    # a name is not a usable device.
    if p.system == "Windows" and not p.gpu_archs:
        p.windows_video_controllers = windows_video_controllers()
        p.gpu_archs = archs_from_windows_video_controllers(p.windows_video_controllers)
        if not p.gpu_names:
            p.gpu_names = list(p.windows_video_controllers)
    ...
    p.docker_cli = shutil.which("docker") is not None
    p.rocm_runtime_sonames = rocm_runtime_sonames()
    p.spoofed_devices = detect_spoofed_devices()
    p.capabilities = capabilities_for(p)
    return p


# The sonames the upstream leejet ROCm sd.cpp archive lists as NEEDED while
# bundling none of them: it ships RUNPATH [$ORIGIN] only, so they must come from
# the HOST's ROCm installation. Whether they resolve is the difference between
# "this build has no kernels for your card" and "this machine has no ROCm".
ROCM_RUNTIME_SONAMES = ("libamdhip64.so.7", "libhipblas.so.3", "librocblas.so.5")


def rocm_runtime_sonames(names: tuple[str, ...] = ROCM_RUNTIME_SONAMES) -> dict[str, bool] | None:
    """Can the dynamic loader find each soname? None where the question is not asked.

    `ldconfig -p` rather than dlopen: opening librocblas pulls in a multi-hundred-MB
    Tensile library and can abort the process on a mismatch, and the reporting
    environment has other things to say.
    """
    if platform.system() != "Linux":
        return None
    # ldconfig lives in /sbin, which is off PATH for a non-root user: taking
    # "command not found" for "no ROCm here" would invent the very gap this is
    # meant to measure.
    ...
    if exe is None:
        return None
    try:
        r = subprocess.run([exe, "-p"], capture_output = True, text = True, timeout = 60)
        cache = r.stdout or ""
    except Exception:  # noqa: BLE001
        return None
    return {n: any(line.strip().startswith(n + " ") for line in cache.splitlines())
            for n in names}
...
def capabilities_for(p: "HostProfile") -> dict[str, bool]:
    ...
    sonames = p.rocm_runtime_sonames or {}
    ...
    return {
        ...
        # Windows is reachable on this pool; Docker on Windows is not. Kept as two
        # separate capabilities on purpose: collapsing them back into one "windows"
        # gap would either overstate the reach of a Windows run or understate the
        # reach of the pool. Both are computed, never asserted.
        # The host half of the sd.cpp ROCm prebuilt's dependency. Computed, not
        # asserted, and deliberately BOTH directions: a pool where every box has
        # ROCm can observe that the generic build runs, and cannot observe what it
        # does on the AMD machine that has no ROCm userspace at all -- which is the
        # population a ROCm -> Vulkan fallback exists for.
        "rocm_runtime_libs": bool(sonames) and all(sonames.values()),
        "no_rocm_runtime_host": bool(sonames) and not any(sonames.values()),
        "docker": p.docker_cli,
        "windows_docker": p.system == "Windows" and p.docker_cli,
        "vulkan": any("radeon" in i or "amd" in i for i in p.vulkan_icds),
        # Named gfx targets, so a criteria module can declare "this change is about
        # THAT card" and have the gap computed rather than remembered. Both are
        # False on this pool by construction; they are here so they stay computed
        # if a differently-carded runner ever answers.
        "gfx1201_rdna4": any("gfx1201" in a for a in p.gpu_archs),
        "gfx1100_rdna3": any("gfx1100" in a for a in p.gpu_archs),
        "amdvlk": False,
        "mig": False,
