...
KNOWN_GAPS = {
    ...
    "amd_fp4_matrix_cores": (
        "needs AMD FP4 matrix cores, which exist only on CDNA4 (gfx950, MI355X "
        "class). This runner is gfx1151, RDNA3.5, whose WMMA has no FP4 path at "
        "all, so nothing measured here can speak to MXFP4 or NVFP4 THROUGHPUT on "
        "AMD. A refusal observed on this host is a statement about gfx1151 only: "
        "it is not evidence that AMD lacks FP4, and a future gfx950 run would "
        "answer a question this pool cannot reach"
    ),
    "amd_fp8_matrix_cores": (
        "needs AMD FP8 matrix cores. This runner is gfx1151 (RDNA3.5); whether its "
        "WMMA carries an FP8 path is exactly what the probe measures rather than "
        "asserts, so treat a negative reading as measured-here, not as an AMD-wide "
        "claim. CDNA3 gfx942 and RDNA4 are unreachable from this pool"
    ),
...
TORCH_DERIVED = frozenset({
    ...
    # Both read gpu_archs, which is empty when torch failed to import; without
    # these a torch-less reporting environment would claim gfx1151 "lacks FP4
    # matrix cores" as an observation rather than as a detection failure.
...
def capabilities_for(p: "HostProfile") -> dict[str, bool]:
    ...
    return {
        ...
        # Arch allowlists, not a reading off the device: HIP exposes no "does this
        # part have FP4 matrix cores" query, so the honest form is a named list of
        # the targets that carry them. FP4 block-scaled MFMA (V_MFMA_SCALE_F32_*_f8f6f4)
        # is CDNA4/gfx950 only. FP8 matrix ops are CDNA3 gfx942, CDNA4 gfx950 and
        # RDNA4 gfx120x. RDNA3.5 gfx1151 is on NEITHER list: its WMMA carries
        # FP16/BF16/INT8/INT4 and no float8 or float4 path, which is why a probe
        # measuring an fp8 or fp4 refusal here is describing gfx1151 and not AMD.
        "amd_fp4_matrix_cores": rocm and "gfx950" in archs,
        "amd_fp8_matrix_cores": rocm and any(
            t in archs for t in ("gfx942", "gfx950", "gfx1200", "gfx1201")),
