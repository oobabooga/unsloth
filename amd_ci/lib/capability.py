...
KNOWN_GAPS = {
    ...
    "gfx1033_vangogh": (
        "needs a Van Gogh (gfx1033) APU, the Steam Deck part. This runner is gfx1151 "
        "(Strix Halo), so which torch index an arch gate SELECTS is answerable here "
        "while the wrong-answer arithmetic that motivates the gate -- negative MSE "
        "loss, nan, a failing float64 gradcheck on gfx1033 ROCm wheels -- is not"
    ),
    "glibc_pre_228": (
        "needs a host whose glibc predates 2.28 (CentOS 7, Debian 9, Ubuntu 18.04). "
        "These runners are newer, so only the >=2.28 side of a glibc-floored "
        "constraint is exercised; the fallback window is unmeasured here"
    ),
}
