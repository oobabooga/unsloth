...
class HostProfile:
    ...
    # gfx targets the amdgpu KFD reports, read when torch reported no device on
    # Linux. The kernel's own device list, so it answers with no ROCm userspace.
    kfd_gfx_targets: list[str] = field(default_factory = list)
...
def gfx_target_from_kfd_version(packed: int) -> str:
    """Decode KFD's packed `gfx_target_version`.

    110501 -> gfx1151, as `major * 10000 + minor * 100 + step`. The step is a hex
    digit in the gfx spelling, which is where gfx90a's name comes from. 0 means
    the node is not a GPU (every CPU node reports it).
    """
    if packed <= 0:
        return ""
    major, minor, step = packed // 10000, (packed // 100) % 100, packed % 100
    return f"gfx{major}{minor:x}{step:x}"


def linux_kfd_gfx_targets() -> list[str]:
    """gfx targets from the amdgpu KFD topology, or [] off Linux.

    Torch is how every other arch reading here is taken, and it is a 3 GB install
    that a llama.cpp prebuilt does not otherwise need. The KFD nodes are the
    kernel's own list, cost nothing, and cannot be fooled by the HIP device
    multiplier (which shims the HIP runtime, not sysfs). Used ONLY to fill in an
    arch when torch saw nothing: a node is not proof of a usable runtime, so
    gpu_count stays 0, exactly as the Windows controller fallback leaves it.
    """
        ...
        for props in sorted(glob.glob("/sys/class/kfd/kfd/topology/nodes/*/properties")):
            ...
            for line in text.splitlines():
                ...
                if key != "gfx_target_version":
                    continue
                try:
                    arch = gfx_target_from_kfd_version(int(value.strip()))
                except ValueError:
                    arch = ""
                if arch:
                    out.append(arch)
...
def detect(require_torch: bool = True) -> HostProfile:
    ...
    # Same idea on Linux, from the kernel rather than from a driver name. Lets a
    # job that measures a self-contained llama.cpp bundle gate on gfx1151 without
    # installing a multi-gigabyte torch it will never import.
    if p.system == "Linux" and not p.gpu_archs:
        p.kfd_gfx_targets = linux_kfd_gfx_targets()
        p.gpu_archs = list(p.kfd_gfx_targets)
