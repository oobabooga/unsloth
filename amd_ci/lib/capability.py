...
class HostProfile:
    ...
    # GPU agents as rocminfo lists them, read when torch reported no device on Linux.
    rocminfo_archs: list[str] = field(default_factory = list)
...
ROCMINFO_NAME = re.compile(r"^\s*Name:\s+(gfx[0-9a-z]+)\s*$", re.M)


def archs_from_rocminfo(text: str) -> list[str]:
    """The gfx targets rocminfo lists, one per GPU agent (CPU agents print a model name)."""
    return ROCMINFO_NAME.findall(text or "")


def rocminfo_archs() -> list[str]:
    if platform.system() != "Linux" or shutil.which("rocminfo") is None:
        return []
    try:
        ...
        return []
    return archs_from_rocminfo(r.stdout)
...
def detect(require_torch: bool = True) -> HostProfile:
    ...
    # Arch and count from rocminfo when torch saw nothing; torch_detection stays false.
    if p.system == "Linux" and not p.gpu_archs:
        p.rocminfo_archs = rocminfo_archs()
        if p.rocminfo_archs:
            p.gpu_archs = list(p.rocminfo_archs)
            p.gpu_count = len(p.rocminfo_archs)
            if not p.gpu_names:
                p.gpu_names = list(p.rocminfo_archs)
...
def capabilities_for(p: "HostProfile") -> dict[str, bool]:
    ...
    rocm = bool(p.hip) or bool(p.rocminfo_archs)
