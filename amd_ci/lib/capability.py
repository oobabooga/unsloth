...
KNOWN_GAPS = {
    ...
    "multi_gpu_spoofed": (
        "the HIP device multiplier made torch enumerate extra devices, but they are "
        "the one real GPU wearing other numbers: shared memory, shared compute, no "
...
def untested_section(profile: HostProfile, needed: list[str]) -> str:
    ...
    if profile.spoofed_devices:
        spoof = ["", f"- **{profile.spoofed_devices} of the {profile.gpu_count} devices "
                     f"torch reported are FABRICATED** by the HIP device multiplier. "
                     f"{KNOWN_GAPS['multi_gpu_spoofed']}."]
