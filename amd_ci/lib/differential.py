...
def _truth(value) -> bool:
    """A criterion may return a bool or a (bool, reason) pair; a pair is truthy
    whatever it carries, so unwrap it."""
...
def run_probe(probe: Path, state: str, checkout: str, out_dir: Path,
    ...
    if out.is_file():
        try:
            obs.update(json.loads(out.read_text(encoding = "utf-8")))
...
def main() -> int:
    ...
    if args.fixture:
        ...
        if not fixture_info.get("ready"):
            ...
            print((args.out_dir / "VERDICT.md").read_text(encoding = "utf-8"))
            return 0
    ...
    (args.out_dir / "observations.json").write_text(json.dumps(obs, indent = 2), encoding = "utf-8")

    profile = detect(require_torch = False)
    lines: list[str] = [f"## {args.title or getattr(crit, 'TITLE', 'AMD CI differential')}", ""]
    # Abbreviate a 40-char SHA; leave anything else alone. `tag_states.py` puts
    # release tags here, and "b10079-mi" names no build anybody can download.
    short = {k: (v[:9] if _looks_like_sha(v) else v)
             for k, v in states.get("commits", {}).items()}
    ...
    lines += ["", gap]
    # "this host could answer everything the change needed" is rare, so say it
    # out loud rather than letting an under-declared NEEDS pass for coverage.
    if not profile.torch_detection and (set(needed) & TORCH_DERIVED):
        print("::warning::torch could not be imported, so every GPU capability reads False; "
              "the gaps below are undetermined, not observations about the hardware")
    ...
    report = "\n".join(lines)
    (args.out_dir / "VERDICT.md").write_text(report, encoding = "utf-8")
    (args.out_dir / "verdict.json").write_text(json.dumps(
        {"verdict": verdict, "why": why, "mode": crit.MODE}, indent = 2), encoding = "utf-8")
    print(report)
    return 0
