...
def main() -> int:
    ...
    # Shorten commit shas only; a prebuilt state's label is not one.
    short = {k: (v[:9] if re.fullmatch(r"[0-9a-fA-F]{12,64}", v) else v)
    ...
    if not profile.torch_detection and not profile.gpu_archs and (set(needed) & TORCH_DERIVED):
