...
# Where install.sh lives. A PR in unsloth_zoo (or any other repo) still needs the
# Studio environment for torch/ROCm, and its own checkout cannot provide it.
_INSTALLER_REPO = "https://github.com/unslothai/unsloth"
...
def main() -> int:
    ...
    ap.add_argument("--repo", default = "https://github.com/unslothai/unsloth",
                    help = "upstream the PR number belongs to. unsloth_zoo PRs live in a "
                           "different repository, and leaving this at the default resolved "
                           "the number against unsloth, where it is an unrelated PR")
    ...
    text = (text
            ...
            .replace("__UPSTREAM__", args.repo)
            # Empty when the PR already lives in the installer's repo, so the
            # generated workflow is byte-identical to what it was before the
            # flag existed for every unsloth PR.
            .replace("__ENV_REPO__", "" if args.repo.rstrip("/").endswith("/unsloth")
                     else _INSTALLER_REPO)
