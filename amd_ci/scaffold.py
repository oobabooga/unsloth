...
DEFAULT_UPSTREAM = "https://github.com/unslothai/unsloth"
...
def main() -> int:
    ...
    ap.add_argument("--upstream", default = DEFAULT_UPSTREAM,
                    help = "repository the PR lives in; states.py resolves base/head there")
    ap.add_argument("--env-repo", default = "",
                    help = "repository whose install.sh builds the environment (cloned at its "
                           "default branch); empty = the head state. Required when --upstream "
                           "is not unslothai/unsloth, which is the only repo with install.sh")
    ...
    # The Windows template has no __UPSTREAM__ / __ENV_REPO__ placeholders, so the flags
    # would be accepted and ignored there. Refuse rather than retarget nothing.
    if args.windows and (args.upstream != DEFAULT_UPSTREAM or args.env_repo):
        raise SystemExit("--upstream / --env-repo are Linux-template only")
    # Both Environment steps run install.sh, which only unslothai/unsloth has.
    if args.upstream != DEFAULT_UPSTREAM and not args.env_repo and not (args.no_gpu and args.no_suites):
        raise SystemExit(f"--upstream {args.upstream} has no install.sh; pass --env-repo "
                         f"{DEFAULT_UPSTREAM} so the Environment steps can build")
    ...
    for name in ("workflow.yml", "workflow_windows.yml"):
        shutil.copy(ROOT / "templates" / name, dest / "templates" / name)
    ...
    text = (text
            ...
            .replace("__UPSTREAM__", args.upstream)
            .replace("__ENV_REPO__", args.env_repo)
